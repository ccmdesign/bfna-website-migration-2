...
| Lists | `reset.css:35-40` strips `list-style` from every `ul[class]`/`ol[class]`. `role="list"` was applied in 4 places with a written rationale and missing in **11** — six in `src/components/bf/**`, which the source read found, plus five rendered directly by `index.vue` and `about.vue`, which only the whole-build gate in gh#220 found. All 11 fixed. Measured `/search`: `<ol class="bf-search-shell__results">`, `list-style-type: none`, `role: null`, 20 children |
