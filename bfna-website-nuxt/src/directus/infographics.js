...
const objectContructor = async (dir, fs) => {
  try {
    ...
    const items = await common.getDirectusData("infographics", junctionFields);
    ...
    await items.data.forEach((item) => {
      ...
      i.image = {
        url: item.image ? common.getImage(item.image.id) : null,
      };
      ...
      console.log("WRITING INFOGRAPHICS: ", i.slug + ".json");
    });
  } catch (error) {
...
export const getInfographics = async () => {
  const dir = "./content/infographics";
  ...
  try {
    ...
    console.error('Error in getInfographics:', err);
  }
}
